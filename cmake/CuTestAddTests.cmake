...
function(cutest_discover_tests_impl)

    set(options "")
    set(oneValueArgs
        NO_PRETTY_TYPES   # These two take a value, unlike cutest_discover_tests()
        NO_PRETTY_VALUES  #
        TEST_EXECUTABLE
        TEST_WORKING_DIR
        TEST_PREFIX
        ...
        TEST_XML_OUTPUT_DIR
        # The following are all multi-value arguments in cutest_discover_tests(),
    ...
    if(arg_TEST_FILTER)
        set(filter "--cutest_filter=${arg_TEST_FILTER}")
    else()
        set(filter)
    endif()

    # CMP0178 has already been handled in cutest_discover_tests(), so we only need
    # to implement NEW behavior here. This means preserving empty arguments for
    # TEST_EXECUTOR. For OLD or WARN, cutest_discover_tests() already removed any
    # empty arguments.
    set(launcherArgs "")
    ...
    cmake_language(EVAL CODE
        "execute_process(
            COMMAND ${launcherArgs} [==[${arg_TEST_EXECUTABLE}]==] --cutest_list_tests ${filter} ${discovery_extra_args}
    ...
    foreach(line ${output})
        # Skip header
        if(NOT line MATCHES "cutest_main\\.c")
            ...
            if(NOT line MATCHES "^  ")
                ...
                if(NOT "${arg_TEST_XML_OUTPUT_DIR}" STREQUAL "")
                    set(TEST_XML_OUTPUT_PARAM "--cutest_output=xml:${arg_TEST_XML_OUTPUT_DIR}/${prefix}${suite}.${test}${suffix}.xml")
                else()
                ...
                foreach(arg IN ITEMS
                        "${arg_TEST_EXECUTABLE}"
                        "--cutest_filter=${suite}.${test}"
                        "--cutest_also_run_disabled_tests"
