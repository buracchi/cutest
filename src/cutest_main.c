...
static struct cutest *filter_tests(struct cutest cutest[static 1], size_t n, const char filter[static n]);
...
extern int main(int argc, char *argv[argc + 1]) {
	...
	if (argc > 1 && strncmp(argv[1], "--cutest_filter=", strlen("--cutest_filter=")) == 0) {
		...
		cutest = filter_tests(cutest, strlen(filter) + 1, filter);
		if (cutest == nullptr) {
...
static void run_test_suite(struct cutest_test_suite test_suite[static 1], struct result result[static 1]) {
	...
	printf("[----------] %zu tests from %s\n", test_suite->size, test_suite->name);
...
static struct cutest *filter_tests(struct cutest cutest[static 1], size_t n, const char filter[static n]) {
	struct cutest *cutest_filtered = cutest_init(1, 1);
	if (cutest_filtered == nullptr) {
		return nullptr;
	}
	size_t suite_name_length;
	char *dot_ptr = memchr(filter, '.', n);
	if (dot_ptr == nullptr) {
		return cutest_filtered;
	}
	suite_name_length = dot_ptr - filter;
	for (size_t i = 0; i < cutest->size; i++) {
		struct cutest_test_suite *suite = &cutest->suite[i];
		if (strncmp(suite->name, filter, suite_name_length) == 0
		    && strlen(suite->name) == suite_name_length) {
			const char *required_test = dot_ptr + 1;
			for (size_t j = 0; j < suite->size; j++) {
				struct cutest_test *test = &suite->test[j];
				if (strcmp(test->name, required_test) == 0) {
					if (cutest_test_add(cutest_filtered,
					                    suite->name,
					                    test->name,
					                    test->execute) == nullptr) {
						cutest_destroy(cutest_filtered);
						return nullptr;
					}
					return cutest_filtered;
				}
